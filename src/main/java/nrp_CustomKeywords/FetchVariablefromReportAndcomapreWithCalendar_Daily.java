...
public class FetchVariablefromReportAndcomapreWithCalendar_Daily extends BaseNRP {
	...
	public void fetchDailyReportValueAndComapreWithCalendar(HashMap<String, Object> DateAndValueFromCalendar,ArrayList<String> DayWiseDates,int NoOfColumns,String DataGrain) throws InterruptedException, IOException
	{
		...
		HashMap<String, String> AvgGuestFromReport = new HashMap<String, String>();
		HashMap<String, String> AvgGuestLastYearFromReport = new HashMap<String, String>();
		HashMap<String, String> AvgGuestBudgetFromReport = new HashMap<String, String>();
...

		{
			...
			allvaluesFromCal = DateAndValueFromCalendar;
				...
				Set<String> listForReportColumns = new HashSet<String>(Arrays.asList("Net Sales $", "Net Sales LY $", "Sales Budget $", "Guest Count", "Guest LY Count", "Avg Guest $" , "Avg Guest LY $", "Labor Cost, $", "Labor Cost LY, $"));
			...
			if (user.equalsIgnoreCase("admin.insight360@analytix.com")) {
				...
				for (int i = 0; i < Headers.size(); i++) {
				        ...
				        switch (HeaderName) {  
				            ...
				            case "Guest LY Count":
				               ...
				               log.info(GuestCountLYReport);
				                ...
				                break;
				            case "Avg Guest $":
				                log.info("FETCHING AVG Guest VALUE FROM REPORT FOR DATE:" + (fecthDates.get(DateIndex)));

				                index = i;

				                

				                WebElement AvgGuestReport1 = cellsNew.get(index).findElement(By.tagName("div"));

						        justscrollToElement(AvgGuestReport1);


				                String AvgGuestReport = AvgGuestReport1.getText().replaceAll("\r\n", " ").replaceAll("\r", " ").replaceAll(
				                    "\n", " ");

				               log.info(AvgGuestReport);


				                AvgGuestFromReport.put(fecthDates.get(DateIndex), AvgGuestReport);

				                break;
				            case "Avg Guest LY $":
				                log.info("FETCHING AVG Guest LY VALUE FROM REPORT FOR DATE:" + (fecthDates.get(DateIndex)));

				                index = i;

				              
				                WebElement AvgGuestLYReport1 = cellsNew.get(index).findElement(By.tagName("div"));

						        justscrollToElement(AvgGuestLYReport1);


				                String AvgGuestLYReport = AvgGuestLYReport1.getText().replaceAll("\r\n", " ").replaceAll("\r", " ").replaceAll(
				                    "\n", " ");

				               log.info(AvgGuestLYReport);


				                AvgGuestLastYearFromReport.put(fecthDates.get(DateIndex), AvgGuestLYReport);
				 ...
				 log.info("\"******************EXECUTION FINSISHED AND NOW START COMPARING CALENDAR AND REPORT VALUE FOR SAME PERIOD*******************\"");
					...
					CompareAvgTck = (HashMap<String, String>) ValuesFromCalender.get("AvgGuestValues");

					CompareAvgTckLY = (HashMap<String, String>) ValuesFromCalender.get("AvgGuestLastYearValues");	
					...
					cwc.CompareWithCalender(CompareAvgTck, AvgGuestFromReport,"Average Guest",DataGrain);

					cwc.CompareWithCalender(CompareAvgTckLY,AvgGuestLastYearFromReport,"Average Guest LY",DataGrain);
